...
function get_drop_position(col, row, layer)
    ...
    local target_pos = {pose={
        pallet_frame_drop[1] + offset_x,
        pallet_frame_drop[2] + offset_y,
        pallet_frame_drop[3] + offset_z + box_height, -- Z é o topo da caixa colocada
        pallet_frame_drop[4],
        pallet_frame_drop[5],
        pallet_frame_drop[6]
    }}
...
function process_box()
    ...
    MovJ({pose=approach_drop}, {user = 4, tool = 2, a = 20, v = 20}) -- Movimento articular (rápido) no ar

    -- 5. Posicionamento Fino
    MovL(p_drop, {user = 4, tool = 2, a = 20, v = 20}) -- Movimento linear para precisão na descida
    
    -- 6. Liberação
    print("disable vacuum")   -- Solta vácuo (Porta 1 OFF)
    
    -- 7. Saída Segura
    MovL({pose=approach_drop}, {user = 4, tool = 2, a = 20, v = 20})
end
